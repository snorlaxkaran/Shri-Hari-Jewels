...
export default function AddMotifModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (type === "Motif") {
      if (!motifWeight.trim()) {
        setError("Enter motif weight in grams.");
        return;
      }
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          {type === "Motif" && lockedMetalPurity && (
            <div className="rounded-lg bg-zinc-50 border border-zinc-200 px-3 py-2 text-xs text-zinc-600">
              ...
              . Price is calculated from weight × current market rate (making
              charges are added at the design level).
            </div>
          )}
          {type === "Motif" && !lockedMetalPurity && (
